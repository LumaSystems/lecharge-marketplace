...
> **Design revision (2026-07-21, after Task 5):** the proposal look was reworked to a
> premium document standard (benchmarked against the Kairos `proposals/` references). The
> single `print.css` was replaced by two dedicated print chassis, `proposal-doc.css`
> (vertical A4) and `proposal-deck.css` (landscape 16:9); the generator now asks which
> format the user wants. `render-pdf.mjs` gained `preferCSSPageSize` + a native PDF footer;
> the chassis enforce `break-inside: avoid` so nothing is cut across pages. `COMPONENTS.md`
> is now a two-format catalog. The LeCharge logo was vectorized to SVG and added under
> `packages/lecharge-ui/brand/`. Tasks 1, 3, and 5 below describe the earlier single-format
> approach; the shipped code reflects this revision, and the spec is the current reference.
